defmodule TagIpWeb.EventEditLive do
  ...
  def mount(%{"id" => id}, _session, socket) do
    event = Ash.get!(TagIp.Events.EventDefinition, id, domain: TagIp.Domain)
  ...
  def handle_event("save", %{"event_definition" => params}, socket) do
    event = Ash.get!(TagIp.Events.EventDefinition, socket.assigns.event_id, domain: TagIp.Domain)

    case Ash.update(event, params, domain: TagIp.Domain) do
      {:ok, _updated} ->
      ...
      {:error, error} ->
        ...
        message = Enum.map_join(errors, "; ", fn {_field, msg} -> msg end)
        form = to_form(params, as: :event_definition, errors: errors)
        {:noreply,
         socket
         |> assign(form: form)
         |> put_flash(:error, "Erreur : #{message}")}
    end
  end
  ...
  def handle_event("delete", _params, socket) do
    ...
    case Ash.destroy(event, domain: TagIp.Domain) do
      :ok ->
        Phoenix.PubSub.broadcast(
          TagIp.PubSub,
          "global_events",
